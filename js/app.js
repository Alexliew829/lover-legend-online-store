...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const deepLinkPageMapV210 = {
    ...
    marketing: "marketingPageV27",
    finance: "financePageV27",
    affiliate: "affiliatePageV27",
    more: "morePageV27",
...
function getAccessPasswordSettings() {
  try {
    ...
    return {hash:String(ui.security?.passwordHash||DEFAULT_ACCESS_PASSWORD_HASH),hint:String(ui.security?.passwordHint||DEFAULT_ACCESS_PASSWORD_HINT)};
  } catch (_) {
    return {hash:DEFAULT_ACCESS_PASSWORD_HASH,hint:DEFAULT_ACCESS_PASSWORD_HINT};
  }
}
...
const ONLINE_STORE_V29_FORCE_PASSWORD_DISABLED = true;
...
function setupAccessLock() {
  ...
  const localAccessSettingsV10 = {accessPasswordHash:getAccessPasswordSettings().hash};
...
function setupPasswordChange() {
  ...
  button.addEventListener("click", async () => {
    ...
    const uiSettings = getOnlineStoreUiSettingsV12();
    uiSettings.security = {...uiSettings.security,passwordEnabled:true,passwordHash:newHash,passwordHint:newHint};
    saveOnlineStoreUiSettingsV12(uiSettings);
...
function setupSettings() {
  ...
  if (!window.ONLINE_STORE_IMPORT_READ_ONLY_V20) {
    setupPasswordChange();
...
async function runSystemHealthCheckV203({ refreshSales = false } = {}) {
...
function getEffectiveProductMinimumPriceV333(product, configuredRules = null, originIndex = null) {
  const storedPrice = Math.max(0, Number(product?.importInitialMinimumPrice ?? product?.minimumPrice) || 0);
  if (typeof ONLINE_STORE_IMPORT_READ_ONLY_V20 !== "undefined" && ONLINE_STORE_IMPORT_READ_ONLY_V20) return storedPrice;
...
async function editDisplayedMinimumPriceV199(productId) {
  if (typeof ONLINE_STORE_IMPORT_READ_ONLY_V20 !== "undefined" && ONLINE_STORE_IMPORT_READ_ONLY_V20) return;
...
const ONLINE_STORE_STATE_STORAGE_KEY_V18 = "llaOnlineStoreV22::state";
const ONLINE_STORE_UI_STORAGE_KEY_V18 = "llaOnlineStoreV22::ui";
const ONLINE_STORE_HISTORY_STORAGE_KEY_V18 = "llaOnlineStoreV22::history";
...
function getOnlineStoreUiSettingsV12(){
  let v=loadJSON(ONLINE_STORE_UI_STORAGE_KEY_V18,null);
  if(!v||typeof v!=="object"||Array.isArray(v)){
    const legacy=loadJSON("loverLegendOnlineStoreUiSettingsV18",null);
    const looksOnline=legacy&&typeof legacy==="object"&&!Array.isArray(legacy)&&
      !/进口|Import Cost|Inventory System/i.test(String(legacy.systemNameZh||"")+" "+String(legacy.systemNameEn||"")+" "+String(legacy.browserTitle||""));
    v=looksOnline?legacy:{};
    saveJSON(ONLINE_STORE_UI_STORAGE_KEY_V18,v);
  }
  const defaults={brandName:"Lover Legend Gardening",systemNameZh:"线上商店管理系统",systemNameEn:"Online Store Management Admin",browserTitle:"线上商店管理系统 | Online Store Management Admin"};
  const merged={brandName:String(v.brandName||defaults.brandName),systemNameZh:String(v.systemNameZh||defaults.systemNameZh),systemNameEn:String(v.systemNameEn||defaults.systemNameEn),browserTitle:String(v.browserTitle||defaults.browserTitle),logoLeft:String(v.logoLeft||""),logoRight:String(v.logoRight||""),pricing:{targetMargin:Number(v.pricing?.targetMargin??30),discountRate:Number(v.pricing?.discountRate??10),affiliateRate:Number(v.pricing?.affiliateRate??10),gatewayFee:Number(v.pricing?.gatewayFee??2),packagingCost:Number(v.pricing?.packagingCost??20)},allocation:{randomMinimumQty:Math.max(1,Math.floor(Number(v.allocation?.randomMinimumQty??5)))},security:{passwordEnabled:v.security?.passwordEnabled!==false,passwordHash:String(v.security?.passwordHash||DEFAULT_ACCESS_PASSWORD_HASH),passwordHint:String(v.security?.passwordHint||DEFAULT_ACCESS_PASSWORD_HINT)}};
  if(/进口|Import Cost|Inventory System/i.test(merged.systemNameZh+" "+merged.systemNameEn+" "+merged.browserTitle)){
    merged.systemNameZh=defaults.systemNameZh; merged.systemNameEn=defaults.systemNameEn; merged.browserTitle=defaults.browserTitle;
    saveJSON(ONLINE_STORE_UI_STORAGE_KEY_V18,merged);
  }
  return merged;
}
function saveOnlineStoreUiSettingsV12(v){ saveJSON(ONLINE_STORE_UI_STORAGE_KEY_V18,v); }
function migrateOnlineStoreStorageV22(){
  try{
    if(!localStorage.getItem(ONLINE_STORE_STATE_STORAGE_KEY_V18)){const old=localStorage.getItem("loverLegendOnlineStoreStateV18");if(old)localStorage.setItem(ONLINE_STORE_STATE_STORAGE_KEY_V18,old);}
    if(!localStorage.getItem(ONLINE_STORE_HISTORY_STORAGE_KEY_V18)){const old=localStorage.getItem("loverLegendOnlineStoreHistoryV18");if(old)localStorage.setItem(ONLINE_STORE_HISTORY_STORAGE_KEY_V18,old);}
  }catch(_){}
}
migrateOnlineStoreStorageV22();
...
function onlineStoreDataBoundaryV18(){
  return {
    importReadOnly:["productId","productName","englishName","stock","averageCost","initialMinimumPrice"],
    onlineWritable:["regularPrice","promotionPrice","publishing","rooms","media","orders","customers","affiliate"],
    importWriteBlocked:["stock","averageCost","initialMinimumPrice","minimumPrice","minimumPriceManual","originalCost","importNumber","batch"]
  };
}
...
function renderOnlineStoreProductListV10(){
  ...
  const back=document.getElementById("roomFirstBackV27");
  const banner=document.getElementById("selectedRoomBannerV27");
  const room=onlineStoreSelectedRoomV27;
  if(!room){
    host.hidden=true;if(tools)tools.hidden=true;if(back)back.hidden=true;if(banner)banner.hidden=true;
    host.innerHTML="";return;
  }
  host.hidden=false;if(tools)tools.hidden=false;if(back)back.hidden=false;
...
let onlineStoreRoomSourceV16="parent";
...
function setOnlineStoreEditorValuesV10(productId,preserveRoomV16=false){
  ...
  if(!preserveRoomV16){onlineStoreRoomV16="";onlineStoreRoomSourceV16="parent";onlineStoreRoomDirtyV16=false;}
...
function renderOnlineStoreUniqueItemsV10(items){
  ...
  const renderCard=item=>{
    const tierLabel={entry:"入门首选 / Starter",premium:"精品馆 / Premium",collector:"贵宾室 / VIP Room"}[item.tier]||"入门首选 / Starter";
  ...
  entryHost.innerHTML=groups.entry.map(renderCard).join("")||'<div class="empty-state">尚未建立入门首选的一物一拍 / One-of-a-Kind</div>';
  if(premiumHost)premiumHost.innerHTML=groups.premium.map(renderCard).join("")||'<div class="empty-state">尚未建立精品馆商品</div>';
  if(collectorHost)collectorHost.innerHTML=groups.collector.map(renderCard).join("")||'<div class="empty-state">尚未建立贵宾室商品</div>';
  updateOnlineStoreProfitDisplaysV13();
}
...
function updateOnlineStoreAllocationSummaryV10(){
  ...
  if(warning){warning.hidden=c.unallocated>=0;warning.textContent=c.unallocated<0?`分配超过真实库存 ${Math.abs(c.unallocated)} 棵。请减少随机发货或一物一拍 / One-of-a-Kind 数量后再保存。`:"";}
  return c;
}
...
window.refreshOnlineImportReadOnlyFieldsV21=function(){
  ...
  const pot=document.getElementById("onlineStorePotCostV14");if(pot)pot.value=formatOnlineMoneyInputV12(getOnlineStoreAutomaticPotCostV14(product));
...
function renderStorePreviewV13(){
  ...
  getProducts().filter(p=>!p?.inventoryArchived).forEach(product=>{
    ...
    if(cfg.randomPublished&&cfg.randomQty>0){
      ...
      if(price>0)cards.push({id,name:String(product.name||""),english:productEnglishNameV262(product),price,regular:cfg.regularPrice,promo:cfg.promotionPrice,photo:cfg.randomPhotos?.[0]||"",tier:"入门首选 / Starter",vip:false});
    }
    cfg.uniqueItems.filter(x=>x.published&&x.status!=="sold").forEach(item=>{
      ...
      if(price<=0)return;
      const tier={entry:"入门首选 / Starter",premium:"精品馆 / Premium",collector:"贵宾室 / VIP Room"}[item.tier]||"入门首选 / Starter";
...
function saveOnlineStoreEditorV10(){
  ...
  const randomMinimumV16=getOnlineStoreUiSettingsV12().allocation.randomMinimumQty||5;
  if(config.randomQty>0&&config.randomQty<randomMinimumV16){window.alert(`不能保存：随机发货最低需要 ${randomMinimumV16} 棵。\n\n目前只分配 ${config.randomQty} 棵；可改用入门首选的一物一拍 / One-of-a-Kind。`);return;}
  ...
  const priceChecksV21=[];
  if(config.regularPrice>0)priceChecksV21.push(["随机发货 Online Regular Price",config.regularPrice]);
...
function setupOnlineStoreV10(){
  ...
  list?.addEventListener("click",e=>{const copy=e.target.closest("[data-online-copy-v12]");if(copy){e.preventDefault();e.stopPropagation();copyOnlineTextV12(copy.dataset.onlineCopyV12,copy);return;}const btn=e.target.closest("[data-online-manage-v10]");if(btn){setOnlineStoreEditorValuesV10(btn.dataset.onlineManageV10);if(onlineStoreSelectedRoomV27)openOnlineRoomV16(onlineStoreSelectedRoomV27,"room-first");}});
  ...
  if(!window.__onlineStoreLeaveGuardBoundV21){
    window.__onlineStoreLeaveGuardBoundV21=true;
    const nativeAdd=window.__nativeAddEventListenerOnlineV21||window.addEventListener.bind(window);
    nativeAdd("beforeunload",event=>{if(!onlineStoreRoomDirtyV16&&!window.__onlineStoreSettingsDirtyV21)return;event.preventDefault();event.returnValue="";});
  }
...
const ONLINE_STORE_HISTORY_KEY_V14="onlineStoreHistoryV14"; // legacy key inside Import settings; migration read only
function getOnlineStoreHistoryV14(){
  let list=loadJSON(ONLINE_STORE_HISTORY_STORAGE_KEY_V18,null);
  if(!Array.isArray(list)){
    list=[];
    saveJSON(ONLINE_STORE_HISTORY_STORAGE_KEY_V18,list);
  }
  return list;
}
function addOnlineStoreHistoryV14(type,productId,productName,detail){
  const list=getOnlineStoreHistoryV14().slice();
  list.unshift({ts:new Date().toISOString(),type:String(type||"product"),productId:String(productId||""),productName:String(productName||""),detail:String(detail||"")});
  saveJSON(ONLINE_STORE_HISTORY_STORAGE_KEY_V18,list.slice(0,100));
...
function updateShopPerformanceV27(){
  ...
  set("shopConfiguredProductsV27",formatNumber(configured));
  set("shopOrdersTodayV27","0");set("shopSalesTodayV27","RM 0.00");set("shopPendingTasksV27","0");
}
function setupOnlineStoreAdminV27(){
  const chooseRoom=room=>{
    if(onlineStoreRoomDirtyV16&&!confirmRoomLeaveV16())return;
    const editor=document.getElementById("onlineStoreEditorV10");if(editor)editor.hidden=true;
    onlineStoreSelectedProductIdV10="";onlineStoreRoomV16="";onlineStoreRoomDirtyV16=false;updateOnlineRoomFocusV17();
    onlineStoreSelectedRoomV27=room;
    document.querySelectorAll("[data-room-first-v27]").forEach(b=>b.classList.toggle("active",b.dataset.roomFirstV27===room));
    renderOnlineStoreProductListV10();
  ...
  document.getElementById("langEnglishV27")?.addEventListener("click",()=>{
    ...
    const box=document.createElement("div");box.id="englishComingSoonV27";box.className="english-coming-soon-v27";box.innerHTML='<div><strong>English Version</strong><span>Coming Soon</span><small>中文版完成并稳定后，再一次复制完整功能到 English。</small><button type="button">关闭</button></div>';
...
function setupOnlineStoreSettingsV12(){
  ...
  document.getElementById("onlineStoreBackupV12")?.addEventListener("click",e=>{
    ...
    a.href=url;a.download=`Lover_Legend_Online_Store_Backup_${pad(d.getDate())}-${pad(d.getMonth()+1)}-${d.getFullYear()}.json`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
    e.currentTarget.textContent="已备份";setTimeout(()=>{if(e.currentTarget.isConnected)e.currentTarget.textContent="Backup";},1800);
  });
  const restoreInput=document.getElementById("onlineStoreRestoreFileV18");
  document.getElementById("onlineStoreRestoreV12")?.addEventListener("click",()=>{if(!confirm("Restore 会覆盖当前 Online Store 销售/商城设置。\n\n不会修改 Import 库存、成本或 Import 最低售价。确认继续吗？"))return;if(restoreInput){restoreInput.value="";restoreInput.click();}});
...
\
关闭后任何能打开此网址的人都可进入后台，建议只用于测试。"))return;if(!confirm("再次确认：关闭密码保护？"))return;ui.security={...ui.security,passwordEnabled:false};}else{if(!confirm("重新开启 Online Store 后台密码保护？\
\
下一次进入将需要当前密码。"))return;ui.security={...ui.security,passwordEnabled:true};}saveOnlineStoreUiSettingsV12(ui);addOnlineStoreHistoryV14("settings","","",enabled?"关闭后台密码保护":"开启后台密码保护");refreshPasswordUiV16();e.currentTarget.textContent=enabled?"已关闭":"已开启";setTimeout(refreshPasswordUiV16,1200);});
...
function enforceOnlineImportNumericReadOnlyV28(){
  ["onlineStoreAverageCostV11","onlineStoreMinimumPriceV11","onlineStorePriceFloorV21"].forEach(id=>{
...
function setupHolidayModeV29(){
  ...
  let cfg={enabled:false,start:"",end:"",message:""};
  try{cfg={...cfg,...JSON.parse(localStorage.getItem(ONLINE_STORE_HOLIDAY_KEY_V29)||"{}")};}catch(_){}
  toggle.checked=!!cfg.enabled;if(start)start.value=cfg.start||"";if(end)end.value=cfg.end||"";if(msg)msg.value=cfg.message||"";
  const paint=()=>{const label=toggle.closest("label")?.querySelector("strong");if(label){label.textContent=toggle.checked?"已开启":"关闭";label.classList.toggle("status-danger-v29",toggle.checked);}};paint();toggle.addEventListener("change",paint);
  save.addEventListener("click",()=>{
    save.disabled=true;save.classList.add("is-busy-v29");save.textContent="保存中…";
    const next={enabled:!!toggle.checked,start:String(start?.value||"").trim(),end:String(end?.value||"").trim(),message:String(msg?.value||"").trim()};
    localStorage.setItem(ONLINE_STORE_HOLIDAY_KEY_V29,JSON.stringify(next));
    window.__onlineStoreSettingsDirtyV21=false;addOnlineStoreHistoryV14("settings","","",`Holiday Mode：${next.enabled?"开启":"关闭"}`);
    setTimeout(()=>{save.disabled=false;save.classList.remove("is-busy-v29");save.classList.add("is-success-v29");save.textContent="已保存";setTimeout(()=>{if(save.isConnected){save.classList.remove("is-success-v29");save.textContent="保存 Holiday Mode";}},1300);},250);
  });
}
function enhanceActionButtonStatesV29(){
  const refresh=document.getElementById("refreshImportAnomalyV201");
