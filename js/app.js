...
function setupAccessLock() {
  ...
  if (getOnlineStoreUiSettingsV12().security?.passwordEnabled === false) {
...
function renderSystemInformationV203() {
  ...
  set("systemInfoVersionV203", "V2.4");
...
function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.register("./sw.js?v=22-online-identity-isolated").then(reg=>reg.update()).catch(error => {
...
// ================= Lover Legend Online Store V2.4 =================
...
function onlineStoreDataBoundaryV18(){
  return {
    importReadOnly:["productId","productName","englishName","stock","averageCost","minimumPrice"],
    onlineWritable:["regularPrice","promotionPrice","publishing","rooms","media","orders","customers","affiliate"],
    importWriteBlocked:["stock","averageCost","minimumPrice","minimumPriceManual","originalCost","importNumber","batch"]
  };
}
...
function getOnlineStoreConfigV10(productId){
  ...
  // V2.4 strict boundary: Online Store state stores sales-only fields.
    ...
    return `<article class="online-store-product-card-v10 ${configured?"online-store-configured-v10":""}" data-online-product-v10="${escapeHTML(id)}">
      <div class="online-store-product-main-v10">
        ...
        <div class="online-store-product-meta-v10 online-store-inventory-meta-v12">
          ...
          <span><i>Import 最低售价</i><strong class="online-min-price-v13">${formatMoney(Number(p.minimumPrice)||0,"RM ")}</strong></span>
...
function getOnlineStoreLiveCostConfigV14(product){
  ...
  // V2.4: pot cost is controlled by Import V37.2. VND averageCost excludes the pot,
...
function getImportBaseMinimumPriceV21(product){return Math.max(0,Number(product?.minimumPrice)||0);}
...
function updateOnlineProtectionFloorV21(){
  ...
  if(warning){warning.hidden=!below.length;warning.textContent=below.length?`售价低于 Online 销售保护底线 RM ${formatOnlineMoneyInputV12(info.floor)}。此底线以 Import 最低售价 RM ${formatOnlineMoneyInputV12(info.baseMinimum)} 加上 Online 专属费用计算；请调整售价后再保存。`:"";}
}
...
function saveOnlineStoreEditorV10(){
  ...
  try{
    // V2.4: Import / Inventory System is the only real product + stock + average-cost + minimum-price source.
...
function setupOnlineStoreV10(){
  ...
  document.querySelector('.nav-btn[data-page="onlineStorePage"]')?.addEventListener("click",()=>{
    // V2.4: refresh Import V37.2 read-only stock/cost/minimum-price before repainting Online Store.
  ...
  document.getElementById("onlineStoreCloseEditorV10")?.addEventListener("click",()=>{if(!confirmRoomLeaveV16())return;const e=document.getElementById("onlineStoreEditorV10");if(e)e.hidden=true;onlineStoreSelectedProductIdV10="";onlineStoreRoomV16="";onlineStoreRoomDirtyV16=false;updateOnlineRoomFocusV17();});
  ...
  const mediaNotReadyV14=()=>alert("相册 / 拍照入口已加入。V2.4 尚未连接 Cloudinary 等图片储存服务，为避免图片写入 Google Sheet 超过 50,000 字限制，目前不会把本机照片直接存进库存同步资料。请暂时使用 Photo URL；连接媒体储存后即可永久上传。");
...
function setupOnlineStoreSettingsV12(){
  ...
  document.getElementById("onlineStoreBackupV12")?.addEventListener("click",e=>{
    ...
    const payload={system:"Lover Legend Online Store",version:"2.2",exportedAt:new Date().toISOString(),state:getOnlineStoreStateV10(),ui:getOnlineStoreUiSettingsV12(),history:getOnlineStoreHistoryV14()};
  ...
  restoreInput?.addEventListener("change",async e=>{
    ...
    try{
      ...
      saveJSON(ONLINE_STORE_STATE_STORAGE_KEY_V18,{version:"2.2",products:data.state.products||{}});
  ...
  const refreshPasswordUiV16=()=>{const ui=getOnlineStoreUiSettingsV12(),enabled=ui.security?.passwordEnabled!==false,state=document.getElementById("onlineStorePasswordStateV16"),btn=document.getElementById("onlineStorePasswordToggleV16"),info=document.getElementById("systemInfoPasswordV16");if(state){state.textContent=enabled?"已开启":"已关闭（测试模式）";state.classList.toggle("warning-red-v16",!enabled);}if(btn){btn.textContent=enabled?"关闭密码":"开启密码";btn.classList.toggle("danger-outline-btn",enabled);}if(info){info.textContent=enabled?"已开启":"已关闭 · 测试模式";info.classList.toggle("warning-red-v16",!enabled);}};
